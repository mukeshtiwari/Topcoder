...
function main() {
  try {
    ...
    var reader = new tc.Reader(input);
        var leaves = reader.next('int[]'); reader.next();
    var numberOfLeaves = reader.next('int');

    var SwappingNodes = require("./SwappingNodes");
    var result = SwappingNodes.swapNodes(leaves, numberOfLeaves);
